...
The integration of a customisable widget, either on the user's home screen, control panel or notification centre, can increase accessibility to the interface, and this could be exemplified through a floating ChatGPT button. With this, the user has immediate access to ask queries or display one of their existing chats for multi-tasking purposes. Primarily, the widget's adaptability is accentuated by its "drag-and-drop" feature, so that users can strategically place it according to their needs and ergonomic advantages. Furthermore, there is less overhead time in loading and launching the application, since users are able to access the ChatGPT prompt bar from multiple locations on their device. Such customisable widgets represent a user-centric approach to interface design, especially for an iOS device, where the use of Siri and Apple Shortcuts can be configured for even more advanced workflows.

### 3.1.4 Implement consistency check on AI-generated responses

When using AI-based systems, the need for coherence and reliable information requires some sort of consistency check. The inherent nature of ChatGPT's current model, GPT3.5, may occasionally generate conflicting responses, so an automated consistency check through the entirety of the conversation could combat this. Leveraging the use of the large language model (LLM) processing capabilities, the system can identify statements that propose direct contradictions with one another. It is then up to the user to pinpoint the more accurate responses amongst the conflicted ones. This not only trains the user's mental model to be more resilient, but also offers ChatGPT the opportunity to build an impressive dataset to refine the underlying model even further.

### 3.1.5 Merge conversations on related topics

When users explore or query about a topic across the span of several conversations, related content is often lost amidst chats. Conversation merging can therefore offer a solution, in which users have the ability to select two or more conversations which focussed around a common topic and combine them into a single chat. This however, requires the analysis of conversations to identify any duplicated information. If this is the case, ChatGPT can either prompt the user, or they themselves have the choice to unite chats that they have selected. It would also be beneficial to allow the user to toggle between a merged and individual original conversations view, helping the them consolidate information on said topic, reduces fragmentation and discourages context switching. If privacy protocols permit, a further advancement of this feature would be to allow merging of chats from different devices or accounts.
